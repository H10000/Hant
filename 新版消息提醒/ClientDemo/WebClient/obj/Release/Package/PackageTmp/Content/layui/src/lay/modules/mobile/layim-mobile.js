...
layui.define(["laytpl", "upload-mobile", "layer-mobile", "zepto"], function (i) {
    ...
    var p = function (i) {
        ...
        v = ['<div class="layim-chat layim-chat-{{d.data.type}}">', '<div class="layim-chat-main">', "<ul></ul>", "</div>", '<div class="layim-chat-footer">', '<div class="layim-chat-send"><span class="layui-icon layim-tool-image" style="font-size: 30px;padding-top:10px;padding-right:5px;" id="btn_input_type">&#xe688;</span><input type="text" autocomplete="off"><button class="layim-send layui-disabled" layim-event="send">发送</button></div>', '<div class="layim-chat-tool" data-json="{{encodeURIComponent(JSON.stringify(d.data))}}">', '<span class="layui-icon layim-tool-face" title="选择表情" layim-event="face">&#xe60c;</span>', "{{# if(d.base && d.base.uploadImage){ }}", '<span class="layui-icon layim-tool-image" title="上传图片" layim-event="image">&#xe60d;<input type="file" name="file" accept="image/*"></span>', "{{# }; }}", "{{# if(d.base && d.base.uploadFile){ }}", '<span class="layui-icon layim-tool-image" title="发送文件" layim-event="image" data-type="file" >&#xe61d;<input type="file" name="file"></span>', "{{# }; }}", "{{# layui.each(d.base.tool, function(index, item){ }}", '<span class="layui-icon  {{item.iconClass||""}} layim-tool-{{item.alias}}" title="{{item.title}}" layim-event="extend" lay-filter="{{ item.alias }}">{{item.iconUnicode||""}}</span>', "{{# }); }}", "</div>", "</div>", "</div>"].join(""),
        v2 = ['<div class="layim-chat-system"><span id="chatHistoryLog">查看历史记录</span ></div>', '{{# layui.each(d.data, function(index, item){ }} <div class="layui-card"> <div class="layui-card-header" style="color:#01AAED;"> ', '{{ item.SendTime }} </div>', '<div class="layui-card-body"> {{ item.MessageContent }} </div> </div>{{# })}} '].join(""),
        v3 = ['{{# layui.each(d.data, function(index, item){ }} <div class="layui-card"> <div class="layui-card-header" style="color:#01AAED;"> ', '{{ item.SendTime }} </div>', '<div class="layui-card-body"> {{ item.MessageContent }} </div> </div>{{# })}} '].join(""),
        g = function (i) {
            return i < 10 ? "0" + (0 | i) : i
        };
        ...
        U = {
            chat: function (i) {
                ...
                var e = layui.data("layim-mobile")[C.mine.id] || {},
                    ...
                    s = {};
                if (String(n).indexOf('Sys') != -1) {
                    s = (e.history || {})[n] || {};
                    a(".layim-" + s.type + s.id).find(".layim-msg-status").removeClass(o);
                    a("#LAY_layimNewMsg").removeClass(o);
                    delete C.message[s.type + s.id];
                    //弹出一个面板
                    openSystem(s.id, s.name, v2, v3);
                }
                else {
                    currentCID = ((e.history || {})[n] || {}).id;
                    currentName = ((e.history || {})[n] || {}).name;
            ...
            image: function (i) {
                ...
                l({
                    ...
                    success: function (i) {
                        0 == i.code ? (
                        i.data = i.data || {},
                        "images" === e ? R(t.textarea[0], "img[" + (i.data.src || "") + "]")
                        : "file" === e && R(t.textarea[0], "file(" + (i.data.src || "") + ")[" + (i.data.name || "下载文件") + "]"), q()
                        ) : n.msg(i.msg || "上传失败")
                    }
                })
            },
            ...
            playAudio: function (i) {
                var vodio = document.getElementsByTagName("audio");
                if (vodio.length > 0) {
                    document.removeChild(vodio);
                }
                var e = undefined,//i.data("audio"),
                    a = document.createElement("audio"),// e || document.createElement("audio"),
                    t = function () {
            ...
            about: function () {
                n.open({
                    content: '<p style="padding-bottom: 5px;">身无彩凤双飞翼，心有灵犀一点通。</p>',

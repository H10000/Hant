    ...
    public class WebSocket
    {
        ...
        private void Accept(IAsyncResult socket)
        {
            ...
            try
            {
                ...
                //session.IP =((IPEndPoint)SockeClient.RemoteEndPoint).Address.ToString();
                //session.Port = ((IPEndPoint)SockeClient.RemoteEndPoint).Port;
            ...
            try
            {
                ...
                if (SockeClient.Connected)
                {
                    ...
                    if (msg.Contains("Sec-WebSocket-Key"))
                    {
                        ...
                        //保存账号信息
                        string UserID = msg.Split('/')[1].Split(' ')[0];
                        SessionPool[IP].UserID = UserID;
                        return;
                    }
                    if (SessionPool[IP].isWeb)
                    {
                        ...
                        Messaage ms = JsonConvert.DeserializeObject<Messaage>(msg);
                        byte[] msgBuffer = PackageServerData(ms.Message);
                        foreach (Session se in SessionPool.Values)
                        {
                            if (se.UserID == ms.ReceiveID)
                            {
